...
catg_links = [('https://shopliga.ru' + link['href'], link.text)
              for link in catg_links]  # список ссылок
# на веб-страницы с категориями
...
def get_products(url):
    ...
    :param url: Ссылка на веб-страницу с таблицей товаров.
    :return: Список найденных на веб-странице товаров.
...
if q == '1':
    f = open('C:\Program Files\Python\Frameworks\\telegrambot2022\\'
             'finance_telebot\data_define\data_set_ex.txt',
             'a', encoding='utf-8')
elif q == '2':
    f = open('C:\Program Files\Python\Frameworks\\telegrambot2022\\'
             'finance_telebot\data_define\data_set_in.txt',
             'a', encoding='utf-8')
...
for link, catg in catg_links:
    ...
    subcatg_links = subsoup.select('div.col-md-3 > div > div > '
                                   'ul > li > a')
    # поиск всех подкатегорий в категории
    if subcatg_links:  # если подкатегории найдены, то создаём список
        # с ссылками на них,
        subcatg_links = [('https://shopliga.ru' + link['href'],
                          link.text)
                         for link in subcatg_links]
    else:
        # иначе ищем нужную категорию на основном сайте и собираем
        # соответствующий список оттуда
    ...
    for sublink, subcatg in subcatg_links:
        ...
        print("\t" + subcatg + ':')
        items = get_products(sublink)  # получение списка товаров
        # из подкатегории
        if not items:
            continue

        is_separately = input('Определять категории '
                              'по-отдельности (0 или 1)? ')
        if int(is_separately) > 1 and k <= 0:
            # если введено число больше 1, то оно считывается
            # как значение счётчика пропусков
            k = int(is_separately)

        if int(is_separately) == 1:  # Если введено 1,
            # то каждому предмету нужно будет присваивать метку отдельно
            for item in items:
                label = input('\t\t' + item + ' -> ')
                f.write(f'{item} @ {label}\n')
        elif int(is_separately) == 0:  # если же введено 0,
            # то для всех предметов из списка будет присвоена одна метка
            c_label = input('Введите общую для всех товаров '
                            'категорию: ')
            for item in items:
